...
4. On QA failure, call `dev_env_release` with `reason=qa_failed`.
...
2. If production deployment fails, call `dev_env_release` with `reason=prod_failed`.
3. After production succeeds and the Agent HQ task reaches done, call `dev_env_release` with `reason=done`.
